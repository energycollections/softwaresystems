...
public class MetadataResponse extends AbstractResponse {
    ...
    private static final Field.Array REPLICAS = new Field.Array("replicas", INT32, "The set of all nodes that host this partition.");

    private static final Field.Array ISR = new Field.Array("isr", INT32, "The set of nodes that are in sync with the leader for this partition.");

    private static final Field.Array OFFLINE_REPLICAS = new Field.Array("offline_replicas", INT32, "The set of offline replicas of this partition.");

    private static final Field METADATA_BROKER_V0 = BROKERS.withFields(NODE_ID, HOST, PORT);

    private static final Field PARTITION_METADATA_V0 = PARTITION_METADATA.withFields(ERROR_CODE, PARTITION_ID, LEADER, REPLICAS, ISR);

    private static final Field TOPIC_METADATA_V0 = TOPIC_METADATA.withFields(ERROR_CODE, TOPIC_NAME, PARTITION_METADATA_V0);
    ...
    private static final Field TOPIC_METADATA_V1 = TOPIC_METADATA.withFields(ERROR_CODE, TOPIC_NAME, IS_INTERNAL, PARTITION_METADATA_V0);
    ...
    private static final Schema METADATA_RESPONSE_V3 = new Schema(THROTTLE_TIME_MS, METADATA_BROKER_V1, CLUSTER_ID, CONTROLLER_ID, TOPIC_METADATA_V1);
    ...
    private static final Field PARTITION_METADATA_V5 = PARTITION_METADATA.withFields(ERROR_CODE, PARTITION_ID, LEADER, REPLICAS, ISR, OFFLINE_REPLICAS);

    private static final Field TOPIC_METADATA_V5 = TOPIC_METADATA.withFields(ERROR_CODE, TOPIC_NAME, IS_INTERNAL, PARTITION_METADATA_V5);

    private static final Schema METADATA_RESPONSE_V5 = new Schema(THROTTLE_TIME_MS, METADATA_BROKER_V1, CLUSTER_ID, CONTROLLER_ID, TOPIC_METADATA_V5);
    ...
    private static final Field PARTITION_METADATA_V7 = PARTITION_METADATA.withFields(ERROR_CODE, PARTITION_ID, LEADER, LEADER_EPOCH, REPLICAS, ISR, OFFLINE_REPLICAS);

    private static final Field TOPIC_METADATA_V7 = TOPIC_METADATA.withFields(ERROR_CODE, TOPIC_NAME, IS_INTERNAL, PARTITION_METADATA_V7);

    private static final Schema METADATA_RESPONSE_V7 = new Schema(THROTTLE_TIME_MS, METADATA_BROKER_V1, CLUSTER_ID, CONTROLLER_ID, TOPIC_METADATA_V7);
    ...
    private List<Node> convertToNodes(Map<Integer, Node> brokers, Object[] brokerIds) {
        ...
        for (Object brokerId : brokerIds) if (brokers.containsKey(brokerId))
    ...
    public Map<Errors, Integer> errorCounts() {
        ...
        for (TopicMetadata metadata : topicMetadata) updateErrorCounts(errorCounts, metadata.error);
        return errorCounts;
    }
    ...
    public Cluster cluster() {
        ...
        return new Cluster(this.clusterId, this.brokers, partitions, topicsByError(Errors.TOPIC_AUTHORIZATION_FAILED), topicsByError(Errors.INVALID_TOPIC_EXCEPTION), internalTopics, this.controller);
    }
    ...
    public static class TopicMetadata {
        ...
        public String toString() {
            return "(type=TopicMetadata" + ", error=" + error + ", topic=" + topic + ", isInternal=" + isInternal + ", partitionMetadata=" + partitionMetadata + ')';
        }
    }
        ...
        for (TopicMetadata metadata : topicMetadata) {
            ...
            for (PartitionMetadata partitionMetadata : metadata.partitionMetadata()) {
                ...
                if (partitionData.hasField(OFFLINE_REPLICAS)) {
                    ...
                    for (Node node : partitionMetadata.offlineReplicas) offlineReplicas.add(node.id());
