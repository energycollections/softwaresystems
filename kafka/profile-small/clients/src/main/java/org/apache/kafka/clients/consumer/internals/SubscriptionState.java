...
public class SubscriptionState {
    ...
    private final List<Listener> listeners = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();
    ...
    public void assignFromSubscribed(Collection<TopicPartition> assignments) {
        ...
        if (this.subscribedPattern != null) {
            ...
            for (TopicPartition tp : assignments) if (!this.subscription.contains(tp.topic()))
    ...
    private <T extends Collection<TopicPartition>> T collectPartitions(Predicate<TopicPartitionState> filter, Collector<TopicPartition, ?, T> collector) {
        return assignment.stream().filter(state -> filter.test(state.value())).map(PartitionStates.PartitionState::topicPartition).collect(collector);
    }
    ...
    public Set<TopicPartition> partitionsNeedingReset(long nowMs) {
        return collectPartitions(state -> state.awaitingReset() && state.isResetAllowed(nowMs), Collectors.toSet());
    }
    ...
    public void resetFailed(Set<TopicPartition> partitions, long nextRetryTimeMs) {
        for (TopicPartition partition : partitions) assignedState(partition).resetFailed(nextRetryTimeMs);
    }
    ...
    private static Map<TopicPartition, TopicPartitionState> partitionToStateMap(Collection<TopicPartition> assignments) {
        ...
        for (TopicPartition tp : assignments) map.put(tp, new TopicPartitionState());
        return map;
    }
