...
public class NodeApiVersions {
    ...
    private final List<ApiVersion> unknownApis = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();
    ...
    private short latestUsableVersion(ApiKeys apiKey, ApiVersion supportedVersions, short minAllowedVersion, short maxAllowedVersion) {
        ...
        if (minVersion > maxVersion)
            throw new UnsupportedVersionException("The broker does not support " + apiKey + " with version in range [" + minAllowedVersion + "," + maxAllowedVersion + "]. The supported" + " range is [" + supportedVersions.minVersion + "," + supportedVersions.maxVersion + "].");
        return maxVersion;
    }
