...
public class MockConsumer<K, V> implements Consumer<K, V> {
    ...
    public MockConsumer(OffsetResetStrategy offsetResetStrategy) {
        ...
        this.paused = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
        this.closed = false;
        ...
        this.committed = new org.apache.commons.collections4.map.HashedMap<>();
    }
    ...
    public synchronized void commitAsync(Map<TopicPartition, OffsetAndMetadata> offsets, OffsetCommitCallback callback) {
        ensureNotClosed();
        for (Map.Entry<TopicPartition, OffsetAndMetadata> entry : offsets.entrySet()) committed.put(entry.getKey(), entry.getValue());
        if (callback != null) {
    ...
    public synchronized void seekToBeginning(Collection<TopicPartition> partitions) {
        ensureNotClosed();
        for (TopicPartition tp : partitions) subscriptions.requestOffsetReset(tp, OffsetResetStrategy.EARLIEST);
    }
    ...
    public synchronized void seekToEnd(Collection<TopicPartition> partitions) {
        ensureNotClosed();
        for (TopicPartition tp : partitions) subscriptions.requestOffsetReset(tp, OffsetResetStrategy.LATEST);
    }
