...
public class TransactionManager {
    ...
    private enum Priority {
        ...
        FIND_COORDINATOR(0), INIT_PRODUCER_ID(1), ADD_PARTITIONS_OR_OFFSETS(2), END_TXN(3);
    ...
    public TransactionManager(LogContext logContext, String transactionalId, int transactionTimeoutMs, long retryBackoffMs) {
        ...
        this.nextSequence = new org.apache.commons.collections4.map.HashedMap<>();
        this.lastAckedSequence = new org.apache.commons.collections4.map.HashedMap<>();
        ...
        this.pendingTxnOffsetCommits = new org.apache.commons.collections4.map.HashedMap<>();
        ...
        this.lastAckedOffset = new org.apache.commons.collections4.map.HashedMap<>();
    ...
    public synchronized void failIfNotReadyForSend() {
        if (hasError())
            throw new KafkaException("Cannot perform send because at least one previous transactional or " + "idempotent request has failed with errors.", lastError);
        if (isTransactional()) {
            if (!hasProducerId())
    ...
    synchronized void transitionToAbortableError(RuntimeException exception) {
        if (currentState == State.ABORTING_TRANSACTION) {
            log.debug("Skipping transition to abortable error state since the transaction is already being " + "aborted. Underlying exception: ", exception);
            return;
        }
    ...
    private synchronized void startSequencesAtBeginning(TopicPartition topicPartition) {
        ...
        for (ProducerBatch inFlightBatch : inflightBatchesBySequence.get(topicPartition)) {
            log.info("Resetting sequence number of batch with current sequence {} for partition {} to {}", inFlightBatch.baseSequence(), inFlightBatch.topicPartition, sequence);
        ...
        for (Iterator<TopicPartition> iter = partitionsWithUnresolvedSequences.iterator(); iter.hasNext(); ) {
            ...
            if (!hasInflightBatches(topicPartition)) {
                ...
                if (isNextSequence(topicPartition, sequenceNumber(topicPartition))) {
                    ...
                    log.info("No inflight batches remaining for {}, last ack'd sequence for partition is {}, next sequence is {}. " + "Going to reset producer state.", topicPartition, lastAckedSequence(topicPartition), sequenceNumber(topicPartition));
                    return true;
                }
            }
        }
    ...
    synchronized TxnRequestHandler nextRequestHandler(boolean hasIncompleteBatches) {
        ...
        if (nextRequestHandler.isEndTxn() && !transactionStarted) {
            ...
            if (currentState != State.FATAL_ERROR) {
                log.debug("Not sending EndTxn for completed transaction since no partitions " + "or offsets were successfully added");
                completeTransaction();
            }
    ...
    private synchronized void transitionTo(State target, RuntimeException error) {
        if (!currentState.isTransitionValid(currentState, target)) {
            ...
            throw new KafkaException(idString + "Invalid transition attempted from state " + currentState.name() + " to state " + target.name());
        }
    ...
    private TxnOffsetCommitHandler txnOffsetCommitHandler(TransactionalRequestResult result, Map<TopicPartition, OffsetAndMetadata> offsets, String consumerGroupId) {
        for (Map.Entry<TopicPartition, OffsetAndMetadata> entry : offsets.entrySet()) {
            ...
            CommittedOffset committedOffset = new CommittedOffset(offsetAndMetadata.offset(), offsetAndMetadata.metadata(), offsetAndMetadata.leaderEpoch());
            pendingTxnOffsetCommits.put(entry.getKey(), committedOffset);
        }
        TxnOffsetCommitRequest.Builder builder = new TxnOffsetCommitRequest.Builder(transactionalId, consumerGroupId, producerIdAndEpoch.producerId, producerIdAndEpoch.epoch, pendingTxnOffsetCommits);
        ...
        public void handleResponse(AbstractResponse response) {
            ...
            for (Map.Entry<TopicPartition, Errors> topicPartitionErrorEntry : errors.entrySet()) {
                ...
                if (error == Errors.NONE) {
                    ...
                    fatalError(error.exception());
                    return;
                } else if (error == Errors.INVALID_PRODUCER_ID_MAPPING || error == Errors.INVALID_TXN_STATE) {
                    fatalError(new KafkaException(error.exception()));
                    return;
                } else if (error == Errors.TOPIC_AUTHORIZATION_FAILED) {
                    unauthorizedTopics.add(topicPartition.topic());
                } else if (error == Errors.OPERATION_NOT_ATTEMPTED) {
                    log.debug("Did not attempt to add partition {} to transaction because other partitions in the " + "batch had errors.", topicPartition);
    ...
    private class FindCoordinatorHandler extends TxnRequestHandler {
        ...
        public void handleResponse(AbstractResponse response) {
            ...
            if (error == Errors.NONE) {
                ...
                fatalError(new KafkaException(String.format("Could not find a coordinator with type %s with key %s due to" + "unexpected error: %s", builder.coordinatorType(), builder.coordinatorKey(), findCoordinatorResponse.error().message())));
            }
        }
    }
    ...
    private class TxnOffsetCommitHandler extends TxnRequestHandler {
        ...
        public void handleResponse(AbstractResponse response) {
            ...
            log.debug("Received TxnOffsetCommit response for consumer group {}: {}", builder.consumerGroupId(), errors);
            for (Map.Entry<TopicPartition, Errors> entry : errors.entrySet()) {
                ...
                if (error == Errors.NONE) {
                    ...
                    abortableError(new GroupAuthorizationException(builder.consumerGroupId()));
                    break;
                } else if (error == Errors.TRANSACTIONAL_ID_AUTHORIZATION_FAILED || error == Errors.INVALID_PRODUCER_EPOCH || error == Errors.UNSUPPORTED_FOR_MESSAGE_FORMAT) {
                    fatalError(error.exception());
                    break;
                } else {
