...
public class MockProducer<K, V> implements Producer<K, V> {
    ...
    public MockProducer(final Cluster cluster, final boolean autoComplete, final Partitioner partitioner, final Serializer<K> keySerializer, final Serializer<V> valueSerializer) {
        ...
        this.offsets = new org.apache.commons.collections4.map.HashedMap<>();
        this.sent = new org.eclipse.collections.impl.list.mutable.FastList<>();
        this.uncommittedSends = new org.eclipse.collections.impl.list.mutable.FastList<>();
        this.consumerGroupOffsets = new org.eclipse.collections.impl.list.mutable.FastList<>();
        this.uncommittedConsumerGroupOffsets = new org.apache.commons.collections4.map.HashedMap<>();
        this.completions = new ArrayDeque<>();
        this.mockMetrics = new org.apache.commons.collections4.map.HashedMap<>();
    }
     ...
     */
    public MockProducer(final boolean autoComplete, final Partitioner partitioner, final Serializer<K> keySerializer, final Serializer<V> valueSerializer) {
    ...
    public synchronized Future<RecordMetadata> send(ProducerRecord<K, V> record, Callback callback) {
        ...
        FutureRecordMetadata future = new FutureRecordMetadata(result, 0, RecordBatch.NO_TIMESTAMP, 0L, 0, 0, Time.SYSTEM);
        long offset = nextOffset(topicPartition);
        Completion completion = new Completion(offset, new RecordMetadata(topicPartition, 0, offset, RecordBatch.NO_TIMESTAMP, Long.valueOf(0L), 0, 0), result, callback);
        if (!this.transactionInFlight)
    ...
    public synchronized void flush() {
        verifyProducerState();
        while (!this.completions.isEmpty()) completeNext();
    }
