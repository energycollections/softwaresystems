...
public class CompactionStrategyManager implements INotificationConsumer {
    ...
    private void handleListChangedNotification(Iterable<SSTableReader> added, Iterable<SSTableReader> removed) {
        ...
        try {
            ...
            List<Set<SSTableReader>> repairedRemoved = new org.eclipse.collections.impl.list.mutable.FastList<>(locationSize);
            List<Set<SSTableReader>> repairedAdded = new org.eclipse.collections.impl.list.mutable.FastList<>(locationSize);
            List<Set<SSTableReader>> unrepairedRemoved = new org.eclipse.collections.impl.list.mutable.FastList<>(locationSize);
            List<Set<SSTableReader>> unrepairedAdded = new org.eclipse.collections.impl.list.mutable.FastList<>(locationSize);
    ...
    public AbstractCompactionStrategy.ScannerList getScanners(Collection<SSTableReader> sstables, Collection<Range<Token>> ranges) {
        ...
        try {
            ...
            List<Set<SSTableReader>> repairedSSTables = new org.eclipse.collections.impl.list.mutable.FastList<>();
            List<Set<SSTableReader>> unrepairedSSTables = new org.eclipse.collections.impl.list.mutable.FastList<>();
    ...
    public Collection<Collection<SSTableReader>> groupSSTablesForAntiCompaction(Collection<SSTableReader> sstablesToGroup) {
        ...
        try {
            ...
            for (Map.Entry<Integer, List<SSTableReader>> group : groups.entrySet()) anticompactionGroups.addAll(unrepaired.get(group.getKey()).groupSSTablesForAntiCompaction(group.getValue()));
    ...
    public List<AbstractCompactionTask> getUserDefinedTasks(Collection<SSTableReader> sstables, int gcBefore, boolean validateForCompaction) {
        ...
        try {
            ...
            for (Map.Entry<Integer, List<SSTableReader>> group : repairedSSTables.entrySet()) ret.add(repaired.get(group.getKey()).getUserDefinedTask(group.getValue(), gcBefore));
            for (Map.Entry<Integer, List<SSTableReader>> group : unrepairedSSTables.entrySet()) ret.add(unrepaired.get(group.getKey()).getUserDefinedTask(group.getValue(), gcBefore));
            return ret;
        } finally {
            readLock.unlock();
        }
