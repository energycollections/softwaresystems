...
final class LogRecord {
    ...
    public static LogRecord make(String line) {
        try {
            ...
            return new LogRecord(type, // see comment on CASSANDRA-13294 below
            matcher.group(2) + Component.separator, Long.parseLong(matcher.group(3)), Integer.parseInt(matcher.group(4)), Long.parseLong(matcher.group(5)), line);
    ...
    public static Map<SSTable, LogRecord> make(Type type, Iterable<SSTableReader> tables) {
        ...
        Map<String, SSTable> absolutePaths = new org.apache.commons.collections4.map.HashedMap<>();
    ...
    public static Map<String, List<File>> getExistingFiles(Set<String> absoluteFilePaths) {
        ...
        for (File f : uniqueDirectories) f.listFiles(ff);
        return fileMap;
    }
