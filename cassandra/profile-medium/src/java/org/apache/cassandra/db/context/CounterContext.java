...
public class CounterContext {
    ...
    public boolean hasLegacyShards(ByteBuffer context) {
        ...
        if (localAndGlobalCount < totalCount)
            ...
            return true;
        for (int i = 0; i < localAndGlobalCount; i++) if (context.getShort(context.position() + HEADER_SIZE_LENGTH + i * HEADER_ELT_LENGTH) >= 0)
            // found a local shard
            return true;
        return false;
    }
    ...
    public ByteBuffer clearAllLocal(ByteBuffer context) {
        ...
        if (count == 0)
            ...
            return context;
        List<Short> globalShardIndexes = new org.eclipse.collections.impl.list.mutable.FastList<>(count);
        for (int i = 0; i < count; i++) {
