...
public final class CFMetaData {
    ...
    // REQUIRED
    // internal id, never exposed to user
    ...
    // bytes, long, timeuuid, utf8, etc. This is built directly from clusteringColumns
    public volatile ClusteringComparator comparator;

    // partitioner the table uses
    ...
    // Always of size keyValidator.componentsCount, null padded if necessary
    private volatile List<ColumnDefinition> partitionKeyColumns;

    // Of size comparator.componentsCount or comparator.componentsCount -1, null padded if necessary
    private volatile List<ColumnDefinition> clusteringColumns;

    // Always non-PK, non-clustering columns
    ...
    private void rebuild() {
        ...
        if (isSuper() && isDense()) {
            ...
            for (ColumnDefinition def : partitionKeyColumns) newColumnMetadata.put(def.name.bytes, def);
            for (ColumnDefinition def : clusteringColumns) newColumnMetadata.put(def.name.bytes, def);
    ...
    public CFMetaData asNonCompact() {
        ...
        if (nonCompactCopy == null) {
            nonCompactCopy = copyOpts(new CFMetaData(ksName, cfName, cfId, false, isCounter, false, true, isView, copy(partitionKeyColumns), copy(clusteringColumns), copy(partitionColumns), partitioner, superCfKeyColumn, superCfValueColumn), this);
        }
    ...
    public CFMetaData copy(UUID newCfId) {
        return copyOpts(new CFMetaData(ksName, cfName, newCfId, isSuper(), isCounter(), isDense(), isCompound(), isView(), copy(partitionKeyColumns), copy(clusteringColumns), copy(partitionColumns), partitioner, superCfKeyColumn, superCfValueColumn), this);
    }

    public CFMetaData copy(IPartitioner partitioner) {
        return copyOpts(new CFMetaData(ksName, cfName, cfId, isSuper, isCounter, isDense, isCompound, isView, copy(partitionKeyColumns), copy(clusteringColumns), copy(partitionColumns), partitioner, superCfKeyColumn, superCfValueColumn), this);
    }

    private static List<ColumnDefinition> copy(List<ColumnDefinition> l) {
        List<ColumnDefinition> copied = new ArrayList<>(l.size());
        for (ColumnDefinition cd : l) copied.add(cd.copy());
        return copied;
    }
    ...
    private static PartitionColumns copy(PartitionColumns columns) {
        ...
        for (ColumnDefinition cd : columns) newColumns.add(cd.copy());
    ...
    public ColumnDefinition getDroppedColumnDefinition(ByteBuffer name, boolean isStatic) {
        ...
        return isStatic ? ColumnDefinition.staticDef(this, name, type) : ColumnDefinition.regularDef(this, name, type);
    }
    ...
    public static boolean isNameValid(String name) {
        return name != null && !name.isEmpty() && name.length() <= SchemaConstants.NAME_LENGTH && PATTERN_WORD_CHARS.matcher(name).matches();
    }
    ...
    public CFMetaData validate() throws ConfigurationException {
        ...
        if (isCounter()) {
            for (ColumnDefinition def : partitionColumns()) if (!(def.type instanceof CounterColumnType) && (!isSuper() || isSuperColumnValueColumn(def)))
    ...
    public CFMetaData addOrReplaceColumnDefinition(ColumnDefinition def) {
        ...
        switch(def.kind) {
            ...
            case STATIC:
                PartitionColumns.Builder builder = PartitionColumns.builder();
                for (ColumnDefinition column : partitionColumns) if (!column.name.equals(def.name))
                    builder.add(column);
                builder.add(def);
    ...
    public boolean hasCollectionColumns() {
        for (ColumnDefinition def : partitionColumns()) if (def.type instanceof CollectionType && def.type.isMultiCell())
            return true;
        return false;
    }

    public boolean hasComplexColumns() {
        for (ColumnDefinition def : partitionColumns()) if (def.isComplex())
            return true;
        return false;
    }

    public boolean hasDroppedCollectionColumns() {
        for (DroppedColumn def : getDroppedColumns().values()) if (def.type instanceof CollectionType && def.type.isMultiCell())
            return true;
        return false;
    }
    ...
    public static Set<String> flagsToStrings(Set<Flag> flags) {
        return flags.stream().map(Flag::toString).map(String::toLowerCase).collect(Collectors.toSet());
    }
    ...
    public static class Builder {
        ...
        private final List<Pair<ColumnIdentifier, AbstractType>> partitionKeys = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();

        private final List<Pair<ColumnIdentifier, AbstractType>> clusteringColumns = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();

        private final List<Pair<ColumnIdentifier, AbstractType>> staticColumns = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();

        private final List<Pair<ColumnIdentifier, AbstractType>> regularColumns = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();
        ...
        public CFMetaData build() {
            ...
            for (Pair<ColumnIdentifier, AbstractType> p : regularColumns) builder.add(new ColumnDefinition(keyspace, table, p.left, p.right, ColumnDefinition.NO_POSITION, ColumnDefinition.Kind.REGULAR));
            for (Pair<ColumnIdentifier, AbstractType> p : staticColumns) builder.add(new ColumnDefinition(keyspace, table, p.left, p.right, ColumnDefinition.NO_POSITION, ColumnDefinition.Kind.STATIC));
    ...
    public static class DroppedColumn {
        ...
        public boolean equals(Object o) {
            ...
            return name.equals(dc.name) && kind == dc.kind && type.equals(dc.type) && droppedTime == dc.droppedTime;
        }
        ...
        @Override
        public String toString() {
            return MoreObjects.toStringHelper(this).add("name", name).add("kind", kind).add("type", type).add("droppedTime", droppedTime).toString();
        }
    }
}
