...
public abstract class MultiColumnRestriction implements SingleRestriction {
    ...
    public final boolean hasSupportingIndex(SecondaryIndexManager indexManager) {
        for (Index index : indexManager.listIndexes()) if (isSupportedBy(index))
            return true;
        return false;
    }
    ...
    public static class EQRestriction extends MultiColumnRestriction {
        ...
        protected boolean isSupportedBy(Index index) {
            for (ColumnDefinition column : columnDefs) if (index.supportsExpression(column, Operator.EQ))
                return true;
            return false;
        }
    ...
    public abstract static class INRestriction extends MultiColumnRestriction {
        ...
        @Override
        protected boolean isSupportedBy(Index index) {
            for (ColumnDefinition column : columnDefs) if (index.supportsExpression(column, Operator.IN))
                return true;
            return false;
        }
    ...
    public static class SliceRestriction extends MultiColumnRestriction {
        ...
        public MultiCBuilder appendBoundTo(MultiCBuilder builder, Bound bound, QueryOptions options) {
            ...
            for (int i = 0, m = columnDefs.size(); i < m; i++) {
                ...
                if (reversed != column.isReversedType()) {
                    ...
                    values = new org.apache.commons.collections4.list.NodeCachingLinkedList<ByteBuffer>();
        ...
        @Override
        protected boolean isSupportedBy(Index index) {
            for (ColumnDefinition def : columnDefs) if (slice.isSupportedBy(def, index))
                return true;
            return false;
        }
    ...
    public static class NotNullRestriction extends MultiColumnRestriction {
        ...
        @Override
        protected boolean isSupportedBy(Index index) {
            for (ColumnDefinition column : columnDefs) if (index.supportsExpression(column, Operator.IS_NOT))
                return true;
            return false;
        }
