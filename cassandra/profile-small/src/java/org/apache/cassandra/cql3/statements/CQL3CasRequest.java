...
public class CQL3CasRequest implements CASRequest {
    ...
    private final List<RowUpdate> updates = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();

    private final List<RangeDeletion> rangeDeletions = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();
    ...
    public SinglePartitionReadCommand readCommand(int nowInSec) {
        ...
        if (conditions.isEmpty())
            return SinglePartitionReadCommand.create(cfm, nowInSec, columnFilter, RowFilter.NONE, DataLimits.cqlLimits(1), key, new ClusteringIndexSliceFilter(Slices.ALL, false));
    ...
    public PartitionUpdate makeUpdates(FilteredPartition current) throws InvalidRequestException {
        ...
        for (RangeDeletion upd : rangeDeletions) upd.applyUpdates(current, update);
