...
public abstract class Selection {
    ...
    public boolean containsAComplexColumn() {
        for (ColumnDefinition def : getColumns()) if (def.isComplex())
            return true;
        return false;
    }
    ...
    public Map<ColumnDefinition, Integer> getOrderingIndex(boolean isJson) {
        ...
        for (ColumnDefinition column : orderingIndex.keySet()) jsonOrderingIndex.put(column, columnIndex++);
    ...
    public ResultSet.ResultMetadata getResultMetadata(boolean isJson) {
        ...
        if (orderingIndex != null) {
            for (ColumnDefinition orderingColumn : orderingIndex.keySet()) resultMetadata.addNonSerializedColumn(orderingColumn);
        }
    ...
    protected final int getColumnIndex(ColumnDefinition c) {
        for (int i = 0, m = columns.size(); i < m; i++) if (columns.get(i).name.equals(c.name))
            return i;
        return -1;
    }
    ...
    public String toString() {
        return MoreObjects.toStringHelper(this).add("columns", columns).add("columnMapping", columnMapping).add("metadata", metadata).add("collectTimestamps", collectTimestamps).add("collectTTLs", collectTTLs).toString();
    }
    ...
    public class ResultSetBuilder {
        ...
        public void newRow(DecoratedKey partitionKey, Clustering clustering) {
            ...
            current = new org.apache.commons.collections4.list.NodeCachingLinkedList<>(columns.size());
        ...
        private List<ByteBuffer> getOutputRow() {
            ...
            if (isJson) {
                ...
                if (orderingIndex != null) {
                    for (Integer orderingColumnIndex : orderingIndex.values()) jsonRow.add(outputRow.get(orderingColumnIndex));
                }
                outputRow = jsonRow;
            }
            return outputRow;
    ...
    private static class SelectionWithProcessing extends Selection {
        ...
        public int getResultSetIndex(ColumnDefinition c) {
            ...
            if (index < 0)
                return -1;
            for (int i = 0, m = factories.size(); i < m; i++) if (factories.get(i).isSimpleSelectorFactory(index))
                return i;
            return -1;
        }
