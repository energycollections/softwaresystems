...
public class CreateTypeStatement extends SchemaAlteringStatement {
    ...
    private final List<CQL3Type.Raw> columnTypes = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();
    ...
    public void addToRawBuilder(Types.RawBuilder builder) throws InvalidRequestException {
        builder.add(name.getStringTypeName(), columnNames.stream().map(FieldIdentifier::toString).collect(Collectors.toList()), columnTypes.stream().map(CQL3Type.Raw::toString).collect(Collectors.toList()));
    }
