...
public interface Row extends Unfiltered, Iterable<ColumnData> {
    ...
    public static class Merger {
        ...
        private final List<ColumnData> dataBuffer = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();

        private final ColumnDataReducer columnDataReducer;

        public Merger(int size, int nowInSec, boolean hasComplex) {
            this.rows = new Row[size];
            this.columnDataIterators = new org.apache.commons.collections4.list.NodeCachingLinkedList<>(size);
        ...
        public Row merge(DeletionTime activeDeletion) {
            ...
            return rowInfo.isEmpty() && rowDeletion.isLive() && dataBuffer.isEmpty() ? null : BTreeRow.create(clustering, rowInfo, rowDeletion, BTree.build(dataBuffer, UpdateFunction.<ColumnData>noOp()));
        }
        ...
        private static class ColumnDataReducer extends MergeIterator.Reducer<ColumnData, ColumnData> {
            ...
            public ColumnDataReducer(int size, int nowInSec, boolean hasComplex) {
                ...
                this.versions = new org.apache.commons.collections4.list.NodeCachingLinkedList<>(size);
