...
public abstract class LegacyLayout {
    ...
    private static final ColumnDefinition INVALID_DROPPED_COMPLEX_SUBSTITUTE_COLUMN = new ColumnDefinition("", "", ColumnIdentifier.getInterned(ByteBufferUtil.EMPTY_BYTE_BUFFER, UTF8Type.instance), SetType.getInstance(UTF8Type.instance, true), ColumnDefinition.NO_POSITION, ColumnDefinition.Kind.REGULAR);
    ...
    public static AbstractType<?> makeLegacyComparator(CFMetaData metadata) {
        ...
        if (!metadata.isDense()) {
            ...
            if (hasCollections) {
                ...
                for (CFMetaData.DroppedColumn def : metadata.getDroppedColumns().values()) if (def.type instanceof CollectionType && def.type.isMultiCell())
                    defined.put(bytes(def.name), (CollectionType) def.type);
                for (ColumnDefinition def : metadata.partitionColumns()) if (def.type instanceof CollectionType && def.type.isMultiCell())
    ...
    private static LegacyBound decodeBound(CFMetaData metadata, ByteBuffer bound, boolean isStart, boolean isDeletion) {
        ...
        assert !isStatic || (components.size() >= clusteringSize && all(components.subList(0, clusteringSize), ByteBufferUtil.EMPTY_BYTE_BUFFER::equals));
    ...
    public static ByteBuffer encodeBound(CFMetaData metadata, ClusteringBound bound, boolean isStart) {
        ...
        for (int i = 0; i < clustering.size(); i++) builder.add(clustering.get(i));
        if (isStart)
    ...
    public static ByteBuffer encodeCellName(CFMetaData metadata, ClusteringPrefix clustering, ByteBuffer columnName, ByteBuffer collectionElement) {
        ...
        if (metadata.isSuper()) {
            // This should never be null for supercolumns, see decodeForSuperColumn() above
    ...
    public static void serializeAsLegacyPartition(ReadCommand command, UnfilteredRowIterator partition, DataOutputPlus out, int version) throws IOException {
        ...
        for (LegacyLayout.LegacyCell cell : legacyPartition.cells) {
            ...
            if (cell.isExpiring()) {
                ...
                // timestampOfLastDelete (not used, and MIN_VALUE is the default)
    ...
    private static UnfilteredRowIterator toUnfilteredRowIterator(CFMetaData metadata, DecoratedKey key, LegacyDeletionInfo delInfo, Iterator<LegacyCell> cells, boolean reversed, SerializationHelper helper) {
        ...
        Row staticRow = atoms.hasNext() && atoms.peek().isStatic() ? getNextRow(CellGrouper.staticGrouper(metadata, helper), atoms) : Rows.EMPTY_STATIC_ROW;
    ...
    public static Row extractStaticColumns(CFMetaData metadata, DataInputPlus in, Columns statics) throws IOException {
        ...
        Set<ByteBuffer> columnsToFetch = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>(statics.size());
    ...
    public static RowIterator toRowIterator(final CFMetaData metadata, final DecoratedKey key, final Iterator<LegacyCell> cells, final int nowInSec) {
        SerializationHelper helper = new SerializationHelper(metadata, 0, SerializationHelper.Flag.LOCAL);
    ...
    private static Comparator<LegacyAtom> legacyAtomComparator(CFMetaData metadata) {
        return (o1, o2) -> {
            ...
            if (c1.size() != c2.size() || (o1.isCell() == o2.isCell()) || !equalValues(c1, c2, metadata.comparator)) {
                ...
                clusteringComparison = rt.isCollectionTombstone() ? 0 : metadata.comparator.compare(c1, c2);
            }
            ...
            if (o1.isCell()) {
                ...
                // otherwise, we shouldn't have got a clustering equality
    ...
    public static LegacyCell readLegacyCellBody(CFMetaData metadata, DataInput in, ByteBuffer cellname, int mask, SerializationHelper.Flag flag, boolean readAllAsDynamic) throws IOException, UnknownColumnException {
        ...
        if ((mask & COUNTER_MASK) != 0) {
            // timestampOfLastDelete: this has been unused for a long time so we ignore it
            in.readLong();
            long ts = in.readLong();
    ...
    public static class CellGrouper {
        ...
        public boolean addAtom(LegacyAtom atom) {
            return atom.isCell() ? addCell(atom.asCell()) : addRangeTombstone(atom.asRangeTombstone());
        }
        ...
        public boolean addCell(LegacyCell cell) {
            ...
            if (column == null) {
                ...
                if (column.isPrimaryKeyColumn() && metadata.isCQLTable()) {
                    noSpamLogger.warn("Illegal cell name for CQL3 table {}.{}. {} is defined as a primary key column", metadata.ksName, metadata.cfName, column.name);
                    return true;
                }
    ...
    public static class LegacyBound {
        ...
        public Clustering getAsClustering(CFMetaData metadata) {
            ...
            for (int i = 0; i < bound.size(); i++) values[i] = bound.get(i);
    ...
    public static class LegacyCell implements LegacyAtom {
        ...
        public enum Kind {
            ...
            REGULAR, EXPIRING, DELETED, COUNTER
        }
    ...
    public static class LegacyRangeTombstone implements LegacyAtom {
        ...
        public boolean isRowDeletion(CFMetaData metadata) {
            if (start.collectionName != null || stop.collectionName != null || start.bound.size() != metadata.comparator.size() || stop.bound.size() != metadata.comparator.size())
                return false;
            for (int i = 0; i < start.bound.size(); i++) if (!Objects.equals(start.bound.get(i), stop.bound.get(i)))
                return false;
            return true;
        }
    ...
    public static class LegacyDeletionInfo {
        ...
        public final List<LegacyRangeTombstone> inRowTombstones = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();
    ...
    public static class LegacyRangeTombstoneList {
        ...
        private long serializedSizeCompound(boolean isDense) {
            ...
            for (int i = 0; i < this.size; i++) {
                ...
                for (int j = 0; j < start.bound.size(); j++) startBuilder.add(start.bound.get(j));
                for (int j = 0; j < end.bound.size(); j++) endBuilder.add(end.bound.get(j));
                if (start.collectionName != null)
                    startBuilder.add(start.collectionName.name.bytes);
                if (end.collectionName != null)
                    endBuilder.add(end.collectionName.name.bytes);
