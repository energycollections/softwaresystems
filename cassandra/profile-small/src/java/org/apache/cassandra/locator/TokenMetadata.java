...
public class TokenMetadata {
    ...
    private final Set<InetAddress> leavingEndpoints = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
    ...
    private final Set<Pair<Token, InetAddress>> movingEndpoints = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
    ...
    public TokenMetadata() {
        this(SortedBiMultiValMap.<Token, InetAddress>create(null, inetaddressCmp), HashBiMap.<InetAddress, UUID>create(), Topology.empty(), DatabaseDescriptor.getPartitioner());
    }
    ...
    public TokenMetadata cloneOnlyTokenMap() {
        lock.readLock().lock();
        try {
            return new TokenMetadata(SortedBiMultiValMap.create(tokenToEndpointMap, null, inetaddressCmp), HashBiMap.create(endpointToHostIdMap), topology, partitioner);
        } finally {
            lock.readLock().unlock();
        }
    }
    ...
    public Multimap<InetAddress, Token> getEndpointToTokenMapForReading() {
        ...
        try {
            ...
            for (Map.Entry<Token, InetAddress> entry : tokenToEndpointMap.entrySet()) cloned.put(entry.getValue(), entry.getKey());
            return cloned;
        } finally {
            lock.readLock().unlock();
        }
    ...
    public static class Topology {
        ...
        private Topology(Builder builder) {
            ...
            for (Map.Entry<String, Multimap<String, InetAddress>> entry : builder.dcRacks.entrySet()) dcRackBuilder.put(entry.getKey(), ImmutableMultimap.copyOf(entry.getValue()));
        ...
        private static class Builder {
            ...
            Builder(Topology from) {
                ...
                for (Map.Entry<String, ImmutableMultimap<String, InetAddress>> entry : from.dcRacks.entrySet()) dcRacks.put(entry.getKey(), HashMultimap.create(entry.getValue()));
