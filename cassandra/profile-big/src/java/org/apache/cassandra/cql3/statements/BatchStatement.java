...
public class BatchStatement implements CQLStatement {
    ...
    public void checkAccess(ClientState state) throws InvalidRequestException, UnauthorizedException {
        for (ModificationStatement statement : statements) statement.checkAccess(state);
    }
    ...
    private static void verifyBatchSize(Collection<? extends IMutation> mutations) throws InvalidRequestException {
        ...
        if (size > warnThreshold) {
            Set<String> tableNames = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
            for (IMutation mutation : mutations) {
    ...
    private void verifyBatchType(Collection<? extends IMutation> mutations) {
        if (!isLogged() && mutations.size() > 1) {
            Set<DecoratedKey> keySet = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
            Set<String> tableNames = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
            for (IMutation mutation : mutations) {
    ...
    public static class Parsed extends CFStatement {
        ...
        public void prepareKeyspace(ClientState state) throws InvalidRequestException {
            for (ModificationStatement.Parsed statement : parsedStatements) statement.prepareKeyspace(state);
        }
