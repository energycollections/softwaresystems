...
public class StreamSession implements IEndpointStateChangeSubscriber {
    ...
    private final Map<UUID, StreamReceiveTask> receivers = new org.eclipse.collections.impl.map.mutable.ConcurrentHashMap<>();
    ...
    private void logError(Throwable e) {
        if (e instanceof SocketTimeoutException) {
            if (isKeepAliveSupported())
                logger.error("[Stream #{}] Did not receive response from peer {}{} for {} secs. Is peer down? " + "If not, maybe try increasing streaming_keep_alive_period_in_secs.", planId(), peer.getHostAddress(), peer.equals(connecting) ? "" : " through " + connecting.getHostAddress(), 2 * DatabaseDescriptor.getStreamingKeepAlivePeriod(), e);
            else
    ...
    class KeepAliveTask implements Runnable {
        ...
        public void run() {
            ...
            if (last == null || last.wasSent()) {
                ...
                } catch (// connection handler is closed
                RuntimeException e) {
