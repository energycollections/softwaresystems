...
public class ThriftConversion {
    ...
    public static KsDef toThrift(KeyspaceMetadata ksm) {
        ...
        for (// do not include views
        CFMetaData cfm : // do not include views
        ksm.tables) if (// Don't expose CF that cannot be correctly handle by thrift; see CASSANDRA-4377 for further details
        cfm.isThriftCompatible())
    ...
    private static CFMetaData internalFromThrift(CfDef cf_def, boolean isCreation, Collection<ColumnDefinition> previousCQLMetadata, boolean isDense) throws org.apache.cassandra.exceptions.InvalidRequestException, ConfigurationException {
        ...
        try {
            ...
            if (isCreation) {
                addDefaultCQLMetadata(defs, cf_def.keyspace, cf_def.name, hasKeyAlias ? null : keyValidator, rawComparator, subComparator, defaultValidator, isDense);
            }
    ...
    private static void addDefaultCQLMetadata(Collection<ColumnDefinition> defs, String ks, String cf, AbstractType<?> keyValidator, AbstractType<?> comparator, AbstractType<?> subComparator, AbstractType<?> defaultValidator, boolean isDense) {
    ...
    private static Indexes indexDefsFromThrift(CFMetaData cfm, String ksName, String cfName, AbstractType<?> thriftComparator, AbstractType<?> thriftSubComparator, List<ColumnDef> thriftDefs) {
        if (thriftDefs == null)
            return Indexes.none();
        Set<String> indexNames = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
    ...
    public static ColumnDef toThrift(CFMetaData cfMetaData, ColumnDefinition column) {
        ...
        if (matchedIndex != null) {
            ...
            Map<String, String> filteredOptions = Maps.filterKeys(matchedIndex.options, s -> !IndexTarget.TARGET_OPTION_NAME.equals(s));
