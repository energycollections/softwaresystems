...
public class RestCsrfPreventionFilter extends CsrfPreventionFilterBase {
    ...
    private static final Predicate<String> nonModifyingMethods = m -> Objects.nonNull(m) && NON_MODIFYING_METHODS_PATTERN.matcher(m).matches();

    private Set<String> pathsAcceptingParams = new org.eclipse.collections.impl.set.mutable.UnifiedSet<>();
    ...
    private static interface RestCsrfPreventionStrategy {
        ...
        static final NonceSupplier<HttpServletRequest, String> nonceFromRequestHeader = (r, k) -> r.getHeader(k);

        static final NonceSupplier<HttpServletRequest, String[]> nonceFromRequestParams = (r, k) -> r.getParameterValues(k);

        static final NonceSupplier<HttpSession, String> nonceFromSession = (s, k) -> Objects.isNull(s) ? null : (String) s.getAttribute(k);

        static final NonceConsumer<HttpServletResponse> nonceToResponse = (r, k, v) -> r.setHeader(k, v);
    ...
    private class FetchRequest implements RestCsrfPreventionStrategy {
        ...
        private final Predicate<String> fetchRequest = s -> Constants.CSRF_REST_NONCE_HEADER_FETCH_VALUE.equalsIgnoreCase(s);
