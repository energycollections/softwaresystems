...
public class CutDomain {
    ...
    public CutDomain(Atom[] ca, PDPDistanceMatrix pdpMatrix) {
        ...
        this.ca = ca;
        ndom = 0;
        domains = new org.apache.commons.collections4.list.NodeCachingLinkedList<Domain>();
    }
    ...
    public void cutDomain(Domain dom, CutSites cut_sites, PDPDistanceMatrix pdpMatrix) {
        ...
        if (verbose)
            for (i = 0; i < dom1.nseg; i++) System.out.println(String.format("	F ... from %d to %d", dom1.getSegmentAtPos(i).getFrom(), dom1.getSegmentAtPos(i).getTo()));
        cutDomain(dom1, cut_sites, pdpMatrix);
        if (verbose)
            System.out.println(String.format("  C ... cutr dom2: nseg %d", dom2.nseg));
        if (verbose)
            for (i = 0; i < dom2.nseg; i++) System.out.println(String.format("	F ... from %d to %d", dom2.getSegmentAtPos(i).getFrom(), dom2.getSegmentAtPos(i).getTo()));
