...
public abstract class LocalPDBDirectory implements StructureIOFile {
    ...
    // path on the server, starting with a slash and ending before the 2-char split directories
    private String splitDirURL;

    private String obsoleteDirURL;

    // path to the directory before the 2-char split
    ...
    public LocalPDBDirectory(String path) {
        extensions = new org.apache.commons.collections4.list.NodeCachingLinkedList<String>();
    ...
    protected File downloadStructure(String pdbId) throws IOException {
        ...
        switch(fetchBehavior) {
            case LOCAL_ONLY:
                if (existing == null) {
                    throw new IOException(String.format("Structure %s not found in %s " + "and configured not to download.", pdbId, getPath()));
                } else {
                    return existing;
                }
            case FETCH_FILES:
            ...
            case FETCH_REMEDIATED:
                ...
                if (existing != null) {
                    ...
                    if (lastModified < LAST_REMEDIATION_DATE) {
                        ...
                        logger.warn("Replacing file {} with latest remediated (remediation of {}) file from PDB.", existing, LAST_REMEDIATION_DATE_STRING);
                        existing = null;
                        break;
                    } else {
                        return existing;
            ...
            case FORCE_DOWNLOAD:
                ...
                // downloadStructure(String,String,boolean,File) will download it
                existing = null;
                break;
        }
     ...
     *  @throws IOException
     */
    private File downloadStructure(String pdbId, String pathOnServer, boolean obsolete, File existingFile) throws IOException {
