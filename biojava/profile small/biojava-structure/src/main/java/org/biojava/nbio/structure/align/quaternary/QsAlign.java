...
public class QsAlign {
    ...
    public static QsAlignResult align(List<Subunit> s1, List<Subunit> s2, SubunitClustererParameters cParams, QsAlignParameters aParams) throws StructureException {
        ...
        for (int globalKey : clusterMap.keySet()) {
            ...
            List<Integer> keySet = new org.apache.commons.collections4.list.NodeCachingLinkedList<Integer>(clusterMap.keySet());
            ...
            for (int key : clustSubunitMap.keySet()) {
                ...
                for (Entry<Integer, Integer> pair : clustSubunitMap.get(key).entrySet()) {
                    ...
                    for (Integer eqr : clusterEqrs.get(i)) alignRes1.add(eqr + atomArray1.size());
                    for (Integer eqr : clusterEqrs.get(j)) alignRes2.add(eqr + atomArray2.size());
            ...
            msa.getEnsemble().setAtomArrays(Arrays.asList(new Atom[][] { atomArray1.toArray(new Atom[atomArray1.size()]), atomArray2.toArray(new Atom[atomArray2.size()]) }));
