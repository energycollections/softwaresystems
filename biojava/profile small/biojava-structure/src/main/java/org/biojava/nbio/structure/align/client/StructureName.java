...
public class StructureName implements Comparable<StructureName>, Serializable, StructureIdentifier {
    ...
    private boolean initFromURL(String suffix) {
        try {
            ...
            // Don't bother checking query params here
            chainName = null;
            return true;
    ...
    public static ScopDomain guessScopDomain(String name, ScopDatabase scopDB) {
        List<ScopDomain> matches = new org.apache.commons.collections4.list.NodeCachingLinkedList<ScopDomain>();
        ...
        if (scopMatch.matches()) {
            ...
            for (ScopDomain potentialSCOP : scopDB.getDomainsForPDB(pdbID)) {
                ...
                if (potMatch.matches()) {
                    if (chainName.equals(potMatch.group(2)) || chainName.equals("_") || chainName.equals(".") || potMatch.group(2).equals("_") || potMatch.group(2).equals(".")) {
