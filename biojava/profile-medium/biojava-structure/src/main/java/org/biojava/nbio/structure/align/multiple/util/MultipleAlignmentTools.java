...
public class MultipleAlignmentTools {
    ...
    public static List<String> getSequenceAlignment(MultipleAlignment alignment, final List<Integer> mapSeqToStruct) {
        ...
        for (int b = 0; b < alignment.getBlocks().size(); b++) {
            ...
            for (int pos = 0; pos < alignment.getBlocks().get(b).length(); pos++) {
                globalPos++;
                // true if consecutive with the previous
                boolean gaps = true;
                while (gaps) {
                    gaps = false;
                    ...
                    if (gaps) {
                        for (int str = 0; str < alignment.size(); str++) {
                            if (provisionalChar[str] == ' ') {
                                ...
                                alnSequences.set(str, alnSequences.get(str).concat(String.valueOf(Character.toLowerCase(aa))));
                                previousPos[str]++;
                            } else {
            ...
            // true means no more residues to add
            ...
            for (int pos = 0; pos < alignment.getBlocks().get(b).length(); pos++) {
                ...
                while (gaps) {
                    ...
                    if (gaps) {
                        ...
                        // Append the provisional and update the indices
                        for (int str = 0; str < alignment.size(); str++) {
                            alnSequences.set(str, alnSequences.get(str).concat(String.valueOf(provisionalChar[str])));
                            if (provisionalChar[str] != '-') {
                                previousPos[str] = alignment.getBlocks().get(b).getAlignRes().get(str).get(pos);
    ...
    public static MultipleSequenceAlignment<ProteinSequence, AminoAcidCompound> toProteinMSA(MultipleAlignment msta) throws CompoundNotFoundException {
        ...
        Map<String, Integer> uniqueID = new org.apache.commons.collections4.map.HashedMap<String, Integer>();
    ...
    public static Phylogeny getStructuralTree(MultipleAlignment msta) {
        ...
        Map<String, Integer> alreadySeen = new org.apache.commons.collections4.map.HashedMap<String, Integer>();
        for (int i = 0; i < msta.size(); i++) {
