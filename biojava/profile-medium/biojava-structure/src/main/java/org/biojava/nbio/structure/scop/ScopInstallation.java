...
public class ScopInstallation implements LocalScopDatabase {
    ...
    public ScopInstallation(String cacheLocation) {
        ...
        mirrors = new org.eclipse.collections.impl.list.mutable.FastList<ScopMirror>(1);
        domainMap = new org.apache.commons.collections4.map.HashedMap<String, List<ScopDomain>>();
        sunidMap = new org.apache.commons.collections4.map.HashedMap<Integer, ScopDescription>();
    ...
    private void parseComments(BufferedReader buffer) throws IOException {
        commentsMap = new org.apache.commons.collections4.map.HashedMap<Integer, List<String>>();
        int counter = 0;
        String line;
        while ((line = buffer.readLine()) != null) {
            if (line.startsWith("#"))
    ...
    private void initScopURLs() {
        ...
        ScopMirror alt = new ScopMirror(SCOP_DOWNLOAD_ALTERNATE, "dir.cla.scop.%s.txt", "dir.des.scop.%s.txt", "dir.hie.scop.%s.txt", "dir.com.scop.%s.txt");
        mirrors.add(primary);
        mirrors.add(alt);
    }
}
