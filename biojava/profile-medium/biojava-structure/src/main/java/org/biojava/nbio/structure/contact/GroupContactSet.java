...
public class GroupContactSet implements Serializable, Iterable<GroupContact> {
    ...
    private java.util.Map<Pair<ResidueIdentifier>, GroupContact> contacts;

    public GroupContactSet() {
        contacts = new org.apache.commons.collections4.map.HashedMap<Pair<ResidueIdentifier>, GroupContact>();
    }
    ...
    private Pair<ResidueIdentifier> getResIdPairFromContact(GroupContact groupContact) {
        return new Pair<ResidueIdentifier>(new ResidueIdentifier(groupContact.getPair().getFirst()), new ResidueIdentifier(groupContact.getPair().getSecond()));
    }
}
