...
public class QuatSymmetrySubunits {
    ...
    private List<Point3d[]> caCoords = new org.eclipse.collections.impl.list.mutable.FastList<Point3d[]>();

    private List<Point3d> originalCenters = new org.eclipse.collections.impl.list.mutable.FastList<Point3d>();

    private List<Point3d> centers = new org.eclipse.collections.impl.list.mutable.FastList<Point3d>();

    private List<Vector3d> unitVectors = new org.eclipse.collections.impl.list.mutable.FastList<Vector3d>();

    private List<Integer> folds = new ArrayList<Integer>();

    private List<Integer> clusterIds = new org.eclipse.collections.impl.list.mutable.FastList<Integer>();
    ...
    public QuatSymmetrySubunits(List<SubunitCluster> clusters) {
        ...
        for (int c = 0; c < clusters.size(); c++) {
            for (int s = 0; s < clusters.get(c).size(); s++) {
                ...
                for (int i = 0; i < atoms.length; i++) points[i] = atoms[i].getCoordsAsPoint3d();
                caCoords.add(points);
            }
        }
