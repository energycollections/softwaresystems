...
public class SubstitutionMatrixHelper implements Serializable {
    ...
    private static Map<String, SubstitutionMatrix<AminoAcidCompound>> aminoAcidMatrices = new org.apache.commons.collections4.map.HashedMap<String, SubstitutionMatrix<AminoAcidCompound>>();

    private static Map<String, SubstitutionMatrix<NucleotideCompound>> nucleotideMatrices = new org.apache.commons.collections4.map.HashedMap<String, SubstitutionMatrix<NucleotideCompound>>();
    ...
    private static SubstitutionMatrix<AminoAcidCompound> getAminoAcidMatrix(String file) {
        if (!aminoAcidMatrices.containsKey(file)) {
            aminoAcidMatrices.put(file, new SimpleSubstitutionMatrix<AminoAcidCompound>(AminoAcidCompoundSet.getAminoAcidCompoundSet(), getReader(file), file));
        }
    ...
    private static InputStreamReader getReader(String file) {
        ...
        return new InputStreamReader(SubstitutionMatrixHelper.class.getResourceAsStream(String.format("/%s.txt", resourcePathPrefix + file)));
    }
}
