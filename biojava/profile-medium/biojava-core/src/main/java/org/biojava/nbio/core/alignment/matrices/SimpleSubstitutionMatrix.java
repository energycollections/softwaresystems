...
public class SimpleSubstitutionMatrix<C extends Compound> implements SubstitutionMatrix<C>, Serializable {
    ...
    private SimpleSubstitutionMatrix(CompoundSet<C> compoundSet, Scanner input, String name) {
        ...
        cols = new org.eclipse.collections.impl.list.mutable.FastList<C>();
        StringBuilder descriptionIn = new StringBuilder();
        List<short[]> matrixIn = new org.eclipse.collections.impl.list.mutable.FastList<short[]>();
    ...
    public String getMatrixAsString() {
        ...
        int lengthCompound = compoundSet.getMaxSingleCompoundStringLength(), lengthRest = Math.max(Math.max(Short.toString(min).length(), Short.toString(max).length()), lengthCompound) + 1;
        String padCompound = "%" + Integer.toString(lengthCompound) + "s", padRest = "%" + Integer.toString(lengthRest);
