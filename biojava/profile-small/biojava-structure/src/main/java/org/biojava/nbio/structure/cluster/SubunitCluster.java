...
public class SubunitCluster {
    ...
    public SubunitCluster(Subunit subunit) {
        ...
        for (int i = 0; i < subunit.size(); i++) identity.add(i);
    ...
    public boolean mergeSequence(SubunitCluster other, SubunitClustererParameters params, PairwiseSequenceAlignerType alignerType, GapPenalty gapPenalty, SubstitutionMatrix<AminoAcidCompound> subsMatrix) throws CompoundNotFoundException {
        ...
        PairwiseSequenceAligner<ProteinSequence, AminoAcidCompound> aligner = Alignments.getPairwiseAligner(thisSequence, otherSequence, alignerType, gapPenalty, subsMatrix);
        ...
        logger.info(String.format("SubunitClusters are similar in sequence " + "with %.2f sequence identity and %.2f coverage", sequenceIdentity, sequenceCoverage));
        // If coverage and sequence identity sufficient, merge other and this
        List<Integer> thisAligned = new org.apache.commons.collections4.list.NodeCachingLinkedList<Integer>();
        List<Integer> otherAligned = new org.apache.commons.collections4.list.NodeCachingLinkedList<Integer>();
    ...
    public boolean mergeStructure(SubunitCluster other, SubunitClustererParameters params) throws StructureException {
        ...
        logger.info(String.format("SubunitClusters are structurally similar with " + "%.2f RMSD %.2f coverage", rmsd, structureCoverage));
        // Merge clusters
    ...
    public boolean divideInternally(SubunitClustererParameters clusterParams) throws StructureException {
        ...
        for (int s = 0; s < alignedRes.size(); s++) columns.add(new ArrayList<Integer>(alignedRes.get(s).size()));
        ...
        List<Subunit> newSubunits = new org.apache.commons.collections4.list.NodeCachingLinkedList<Subunit>(subunits.size() * columns.size());
        List<List<Integer>> newSubunitEQR = new org.apache.commons.collections4.list.NodeCachingLinkedList<List<Integer>>(subunits.size() * columns.size());
    ...
    public String toString() {
        return "SubunitCluster [Size=" + size() + ", Length=" + length() + ", Representative=" + representative + ", Method=" + method + "]";
    }
