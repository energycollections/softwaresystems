...
public class CoreSuperimposer implements MultipleSuperimposer {
    ...
    public void superimpose(MultipleAlignment alignment) throws StructureException {
        ...
        if (atomArrays.size() <= reference) {
            throw new IndexOutOfBoundsException(String.format("Invalid reference structure: requested %d but " + "only %d structures.", reference, atomArrays.size()));
        }
        alignment.clear();
        ...
        for (BlockSet bs : alignment.getBlockSets()) {
            ...
            for (int i = 0; i < atomArrays.size(); i++) {
                ...
                List<Atom> atomSet1 = new org.apache.commons.collections4.list.NodeCachingLinkedList<Atom>();
                List<Atom> atomSet2 = new org.apache.commons.collections4.list.NodeCachingLinkedList<Atom>();
