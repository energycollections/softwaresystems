...
public class ChainImpl implements Chain {
    ...
    // the 'public' chain identifier as assigned by authors in PDB files
    ...
    // the 'internal' chain identifier as used in mmCIF files
    ...
    public ChainImpl() {
        ...
        seqResGroups = new org.eclipse.collections.impl.list.mutable.FastList<>();
    ...
    public void addGroup(Group group) {
        ...
        if (pdbResnum != null) {
            ...
            if (pdbResnumMap.containsKey(pdbResnum)) {
                logger.warn("Adding residue {}({}) to chain {} but a residue with same residue number is already present: {}({}). Will add only the aminoacid residue (if any) to the lookup, lookups for that residue number won't work properly.", pdbResnum, group.getPDBName(), getChainID(), groups.get(pdbResnumMap.get(pdbResnum)).getResidueNumber(), groups.get(pdbResnumMap.get(pdbResnum)).getPDBName());
    ...
    public Group[] getGroupsByPDB(ResidueNumber start, ResidueNumber end, boolean ignoreMissing) throws StructureException {
        ...
        List<Group> retlst = new org.apache.commons.collections4.list.NodeCachingLinkedList<>();
    ...
    public String toString() {
        ...
        str.append("total SEQRES length: ").append(getSeqResGroups().size()).append(" total ATOM length:").append(getAtomLength()).append(" residues ").append(newline);
        return str.toString();
    }
    ...
    public List<Group> getAtomLigands() {
        ...
        for (Group g : groups) if (!seqResGroups.contains(g) && !g.isWater())
            ligands.add(g);
        return ligands;
    }
    ...
    public boolean isPureNonPolymer() {
        for (Group g : getAtomGroups()) {
            if (g.isPolymeric() && !g.isHetAtomInFile()) {
                return false;
            }
        }
        return true;
