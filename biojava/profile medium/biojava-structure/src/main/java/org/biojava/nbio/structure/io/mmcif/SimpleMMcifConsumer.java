...
public class SimpleMMcifConsumer implements MMcifConsumer {
     ...
     */
    private java.util.List<List<Chain>> allModels;
    ...
    // needed to link entities, chains and compounds...
    ...
    public void newAtomSite(AtomSite atom) {
        ...
        if (!currentNmrModelNumber.equals(nmrModelNumber)) {
            ...
            currentModel = new org.eclipse.collections.impl.list.mutable.FastList<Chain>();
            currentChain = null;
            currentGroup = null;
        }
        if (currentChain == null) {
        ...
        if (startOfNewChain) {
            ...
            currentGroup.setPDBName(groupCode3);
            currentGroup.setHetAtomInFile(isHetAtomInFile);
        } else // ANTHONY BRADLEY ADDED THIS -> WE ONLY WAN'T TO CHECK FOR ALT LOCS WHEN IT's NOT THE FIRST GROUP IN CHAIN
        {
    ...
    public void documentStart() {
        ...
        // atomCount     		= 0;
        allModels = new org.eclipse.collections.impl.list.mutable.FastList<List<Chain>>();
        currentModel = new org.eclipse.collections.impl.list.mutable.FastList<Chain>();
        entities = new org.eclipse.collections.impl.list.mutable.FastList<Entity>();
        entityPolys = new org.eclipse.collections.impl.list.mutable.FastList<>();
        strucRefs = new org.eclipse.collections.impl.list.mutable.FastList<StructRef>();
        seqResChains = new org.eclipse.collections.impl.list.mutable.FastList<Chain>();
        entityChains = new org.eclipse.collections.impl.list.mutable.FastList<Chain>();
        structAsyms = new org.eclipse.collections.impl.list.mutable.FastList<StructAsym>();
        asymId2entityId = new HashMap<String, String>();
        asymId2authorId = new org.apache.commons.collections4.map.HashedMap<>();
        structOpers = new org.eclipse.collections.impl.list.mutable.FastList<PdbxStructOperList>();
        strucAssemblies = new org.eclipse.collections.impl.list.mutable.FastList<PdbxStructAssembly>();
        strucAssemblyGens = new org.eclipse.collections.impl.list.mutable.FastList<PdbxStructAssemblyGen>();
        entitySrcGens = new org.eclipse.collections.impl.list.mutable.FastList<EntitySrcGen>();
        entitySrcNats = new org.eclipse.collections.impl.list.mutable.FastList<EntitySrcNat>();
        entitySrcSyns = new org.eclipse.collections.impl.list.mutable.FastList<EntitySrcSyn>();
        structConn = new org.eclipse.collections.impl.list.mutable.FastList<StructConn>();
        structNcsOper = new org.eclipse.collections.impl.list.mutable.FastList<StructNcsOper>();
        sequenceDifs = new org.eclipse.collections.impl.list.mutable.FastList<StructRefSeqDif>();
        structSiteGens = new org.eclipse.collections.impl.list.mutable.FastList<StructSiteGen>();
    }
            ...
            for (PdbxStructAssembly psa : strucAssemblies) {
                ...
                if (bioAssemblyId != -1) {
                    ...
                    for (BiologicalAssemblyTransformation transf : transformations) {
                        ...
                        if (c.getEntityType() == EntityType.POLYMER && // don't want them in the macromolecularSize count
    ...
    private void linkEntities() {
        for (int i = 0; i < allModels.size(); i++) {
            for (Chain chain : allModels.get(i)) {
                ...
                if (entityInfo == null) {
                    ...
                    logger.info("Could not find an Entity for entity_id {}, for chain id {}, creating a new Entity.", eId, chain.getId());
                    ...
                    logger.debug("Adding chain with chain id {} (auth id {}) to Entity with entity_id {}", chain.getId(), chain.getName(), eId);
                    entityInfo.addChain(chain);
                    chain.setEntityInfo(entityInfo);
                }
            }
        ...
        for (EntityInfo e : entityInfos) {
            if (e.getChains().isEmpty()) {
                logger.info("Entity {} '{}' has no chains associated to it", e.getMolId() < 0 ? "with no entity id" : e.getMolId(), e.getDescription());
            }
        }
    }
        ...
        for (int model = 0; model < structure.nrModels(); model++) {
            ...
            for (Chain seqResChain : seqResChains) {
                ...
                if (atomChain == null) {
                    ...
                    logger.info("Could not map SEQRES chain with asym_id={} to any ATOM chain. Most likely there's no observed residues in the chain.", seqResChain.getId());
                    continue;
                }
    ...
    private void setStructNcsOps() {
        java.util.List<Matrix4d> ncsOperators = new org.eclipse.collections.impl.list.mutable.FastList<Matrix4d>();
    ...
    public void newRefine(Refine r) {
        ...
        if (pdbHeader.getResolution() != PDBHeader.DEFAULT_RESOLUTION) {
            logger.warn("More than 1 resolution value present, will use last one {} and discard previous {} ", r.getLs_d_res_high(), String.format("%4.2f", pdbHeader.getResolution()));
        }
        try {
        ...
        if (pdbHeader.getRfree() != PDBHeader.DEFAULT_RFREE) {
            logger.warn("More than 1 Rfree value present, will use last one {} and discard previous {} ", r.getLs_R_factor_R_free(), String.format("%4.2f", pdbHeader.getRfree()));
        }
        ...
        if (pdbHeader.getRwork() != PDBHeader.DEFAULT_RFREE) {
            logger.warn("More than 1 R work value present, will use last one {} and discard previous {} ", r.getLs_R_factor_R_work(), String.format("%4.2f", pdbHeader.getRwork()));
        }
    ...
    private void addSites() {
        ...
        for (StructSiteGen siteGen : structSiteGens) {
            ...
            // multiple could be in same site.
