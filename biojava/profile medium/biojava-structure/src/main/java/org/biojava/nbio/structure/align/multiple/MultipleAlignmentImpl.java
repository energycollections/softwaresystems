...
public class MultipleAlignmentImpl extends AbstractScoresCache implements Serializable, MultipleAlignment, Cloneable {
    ...
    public MultipleAlignmentImpl(MultipleAlignmentImpl ma) {
        ...
        if (ma.blockSets != null) {
            ...
            this.blockSets = new org.eclipse.collections.impl.list.mutable.FastList<BlockSet>();
    ...
    public List<BlockSet> getBlockSets() {
        if (blockSets == null)
            blockSets = new org.eclipse.collections.impl.list.mutable.FastList<BlockSet>();
        return blockSets;
    }
    ...
    public List<Integer> getAlignResCounts() {
        ...
        for (BlockSet bs : blockSets) {
            ...
            for (int s = 0; s < size(); s++) alignResCounts.set(s, alignResCounts.get(s) + bscounts.get(s));
        }
        return alignResCounts;
    }
    ...
    public List<Double> getCoverages() {
        ...
        coverages = new org.eclipse.collections.impl.list.mutable.FastList<Double>(size());
        for (int s = 0; s < size(); s++) coverages.add(counts.get(s) / (double) getAtomArrays().get(s).length);
        return coverages;
    }
}
