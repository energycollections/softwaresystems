...
public class AtomContactSet implements Serializable, Iterable<AtomContact> {
    ...
    private java.util.Map<Pair<AtomIdentifier>, AtomContact> contacts;

    private double cutoff;

    public AtomContactSet(double cutoff) {
        this.cutoff = cutoff;
        this.contacts = new org.apache.commons.collections4.map.HashedMap<Pair<AtomIdentifier>, AtomContact>();
    }
    ...
    public boolean hasContact(Atom atom1, Atom atom2) {
        return hasContact(new AtomIdentifier(atom1.getPDBserial(), atom1.getGroup().getChainId()), new AtomIdentifier(atom2.getPDBserial(), atom2.getGroup().getChainId()));
    }
