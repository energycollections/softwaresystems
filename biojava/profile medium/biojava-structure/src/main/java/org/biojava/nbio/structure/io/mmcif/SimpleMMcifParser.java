...
public class SimpleMMcifParser implements MMcifParser {
    ...
    public SimpleMMcifParser() {
        consumers = new org.eclipse.collections.impl.list.mutable.FastList<MMcifConsumer>();
        struct = null;
    }
    ...
    public void parse(BufferedReader buf) throws IOException {
        ...
        // used only to reduce logging statements
    ...
    private void endLineChecks(String category, List<String> loopFields, List<String> lineData, Set<String> loopWarnings) throws IOException {
        ...
        // MMCIF version 5 dates
            ...
            PdbxAuditRevisionHistory history = (PdbxAuditRevisionHistory) buildObject(PdbxAuditRevisionHistory.class.getName(), loopFields, lineData, loopWarnings);
            triggerNewPdbxAuditRevisionHistory(history);
        // MMCIF version 5 dates
        } else if (category.equals("_pdbx_database_status")) {
            PdbxDatabaseStatus status = (PdbxDatabaseStatus) buildObject(PdbxDatabaseStatus.class.getName(), loopFields, lineData, loopWarnings);
            ...
            StructNcsOper sNcsOper = (StructNcsOper) buildObject(StructNcsOper.class.getName(), loopFields, lineData, loopWarnings);
            triggerNewStructNcsOper(sNcsOper);
        } else if (category.equals("_atom_sites")) {
            AtomSites atomSites = (AtomSites) buildObject(AtomSites.class.getName(), loopFields, lineData, loopWarnings);
            ...
            PdbxStructOperList structOper = (PdbxStructOperList) buildObject(PdbxStructOperList.class.getName(), loopFields, lineData, loopWarnings);
    ...
    private Object buildObject(String className, List<String> loopFields, List<String> lineData, Set<String> warnings) {
        ...
        Map<String, Method> methodMap = new org.apache.commons.collections4.map.HashedMap<String, Method>();
        for (Method m : methods) {
            methodMap.put(m.getName(), m);
        }
        // and a map of all the fields so that we can lookup them up later
        Map<String, Field> names2fields = new org.apache.commons.collections4.map.HashedMap<>();
