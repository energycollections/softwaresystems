...
public class AlignmentTools {
    ...
    public static <S, T> Map<S, T> applyAlignment(Map<S, T> alignmentMap, Map<T, S> identity, int k) {
        ...
        List<S> image = new org.eclipse.collections.impl.list.mutable.FastList<S>(preimage);
        for (int n = 1; n < k; n++) {
            // apply alignment
     ...
     *  @return
     */
    public static int getSymmetryOrder(Map<Integer, Integer> alignment, final int maxSymmetry, final float minimumMetricChange) {
     ...
     */
    public static int getSymmetryOrder(Map<Integer, Integer> alignment, Map<Integer, Integer> identity, final int maxSymmetry, final float minimumMetricChange) {
        // currently unmodified
        List<Integer> preimage = new ArrayList<Integer>(alignment.keySet());
        ...
        for (int n = 1; n <= maxSymmetry; n++) {
            ...
            double metric = Math.sqrt((double) deltasSq / numDeltas);
    ...
    public static AFPChain splitBlocksByTopology(AFPChain a, Atom[] ca1, Atom[] ca2) throws StructureException {
        ...
        List<Integer> newBlkLen = new org.eclipse.collections.impl.list.mutable.FastList<Integer>();
        ...
        // Split blocks
        List<int[][]> blocks = new org.eclipse.collections.impl.list.mutable.FastList<int[][]>(newBlkLen.size());
        int oldBlk = 0;
        int pos = 0;
    ...
    public static AFPChain replaceOptAln(AFPChain afpChain, Atom[] ca1, Atom[] ca2, Map<Integer, Integer> alignment) throws StructureException {
        ...
        Arrays.sort(res1);
        List<Integer> blockLens = new org.eclipse.collections.impl.list.mutable.FastList<Integer>(2);
        int optLength = 0;
    ...
    public static <S, T> String toConciseAlignmentString(Map<S, T> alignment, Map<T, S> identity) {
        ...
        Map<S, T> alig = new org.apache.commons.collections4.map.HashedMap<S, T>(alignment);
        // Generate inverse alignment
        Map<S, List<S>> inverse = new org.apache.commons.collections4.map.HashedMap<S, List<S>>();
