...
public class SpaceGroup implements Serializable {
    ...
    // the last +? is for ungreedy matching
    ...
    // in space groups I, C, F or H there are pure cell translations corresponding to recenterings
    private Vector3d[] cellTranslations;

    private AxisAngle4d[] axisAngles;

    // indices of array are transformIds
    ...
    public SpaceGroup(int id, int multiplicity, int primitiveMultiplicity, String shortSymbol, String altShortSymbol, BravaisLattice bravLattice) {
        ...
        transformations = new org.eclipse.collections.impl.list.mutable.FastList<Matrix4d>(multiplicity);
    ...
    private void calcRotAxesAndAngles() {
        ...
        for (int i = 1; i < this.transformations.size(); i++) {
            Matrix3d r = new Matrix3d(transformations.get(i).m00, transformations.get(i).m01, transformations.get(i).m02, transformations.get(i).m10, transformations.get(i).m11, transformations.get(i).m12, transformations.get(i).m20, transformations.get(i).m21, transformations.get(i).m22);
    ...
    private static String formatTransCoef(double c) {
        if (Math.abs((Math.rint(c) - c)) < DELTA) {
            // this is an integer
    ...
    public void setTransfAlgebraic(List<String> transfAlgebraic) {
        ...
        if (transformations == null || transformations.size() == 0)
            transformations = new org.eclipse.collections.impl.list.mutable.FastList<Matrix4d>(transfAlgebraic.size());
