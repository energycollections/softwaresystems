...
public class EntityInfo implements Serializable {
    ...
    public EntityInfo() {
        chains = new org.eclipse.collections.impl.list.mutable.FastList<Chain>();
        chains2pdbResNums2ResSerials = new org.apache.commons.collections4.map.HashedMap<String, Map<ResidueNumber, Integer>>();
        molId = -1;
    }
    ...
    public EntityInfo(EntityInfo c) {
        this.chains = new org.eclipse.collections.impl.list.mutable.FastList<Chain>();
        this.chains2pdbResNums2ResSerials = new org.apache.commons.collections4.map.HashedMap<String, Map<ResidueNumber, Integer>>();
        this.molId = c.molId;
        this.type = c.type;
    ...
    private void initResSerialsMap(Chain c) {
        if (c.getSeqResGroups() == null || c.getSeqResGroups().isEmpty()) {
            logger.warn("No SEQRES groups found in chain with asym_id {}, will use residue numbers as given (no insertion codes, not necessarily aligned). " + "Make sure your structure has SEQRES records and that you use FileParsingParameters.setAlignSeqRes(true)", c.getId());
    ...
    private ResidueNumber findResNumInOtherChains(int i, Chain chain) {
        ...
        for (Chain c : getFirstModelChains()) {
            ...
            if (seqResGroup == null) {
                logger.warn("The SEQRES group is null for index {} in chain with asym_id {}, whilst it wasn't null in chain with asym_id {}", i, c.getId(), chain.getId());
                continue;
            }
