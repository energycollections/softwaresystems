...
public class ModifiedAminoAcidCompoundSet implements CompoundSet<AminoAcidCompound> {
    ...
    private final Map<String, AminoAcidCompound> aminoAcidCompoundCache = new org.apache.commons.collections4.map.HashedMap<String, AminoAcidCompound>();
    ...
    public ModifiedAminoAcidCompoundSet(List<AminoAcidComposition> aaList, Map<Character, Double> aaSymbol2MolecularWeight) {
        ...
        for (AminoAcidComposition aa : aaList) {
            this.aminoAcidCompoundCache.put(aa.getSymbol(), new AminoAcidCompound(null, aa.getSymbol(), aa.getShorName(), aa.getName(), aaSymbol2MolecularWeight.get(aa.getSymbol().charAt(0)).floatValue()));
        }
    }
