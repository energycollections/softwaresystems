...
public class CircleOfTrustSalsa {
    ...
    public CircleOfTrustSalsa(VertexQuery queryService, final int cacheSize) throws Exception {
        ...
        synchronized (CircleOfTrustSalsa.class) {
            if (cache == null) {
                cache = Collections.synchronizedMap(new // LRU
                LinkedHashMap<Integer, ArrayList<Integer>>(// LRU
                cacheSize, // LRU
                1.0f, // LRU
                true) {
    ...
    public void initializeGraph(Collection<Integer> circleOfTrust) {
        ...
        int count = 0;
        // int filtered = 0;
        java.util.List<SalsaVertex> tmpAuth = new org.apache.commons.collections4.list.NodeCachingLinkedList<SalsaVertex>(1 + authEntries.length / 100);
    ...
    public ArrayList<SalsaVertex> topAuthorities(int K, HashSet<Integer> removeList) {
        ...
        Collections.sort(all, new Comparator<SalsaVertex>() {
            ...
            public int compare(SalsaVertex salsaVertex, SalsaVertex salsaVertex1) {
                if (salsaVertex.value < salsaVertex1.value)
                    return 1;
                else
                    return (salsaVertex.value > salsaVertex1.value ? -1 : 0);
            }
        });
    ...
    public static void main(String[] args) throws Exception {
        ...
        while (true) {
            ...
            if (circle.size() > maxCircleSize) {
                ...
                java.util.Set<Integer> filteredCircle = new org.eclipse.collections.impl.set.mutable.UnifiedSet<Integer>();
