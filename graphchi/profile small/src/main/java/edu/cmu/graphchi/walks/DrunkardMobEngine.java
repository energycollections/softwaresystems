...
public class DrunkardMobEngine<VertexDataType, EdgeDataType> {
     ...
     */
    public DrunkardMobEngine(String baseFilename, int nShards, DrunkardFactory<VertexDataType, EdgeDataType> factory) throws IOException {
        createGraphChiEngine(baseFilename, nShards);
        this.drivers = new org.apache.commons.collections4.list.NodeCachingLinkedList<DrunkardDriver>();
     ...
     */
    public DrunkardJob addJob(String jobName, EdgeDirection edgeDirection, WalkUpdateFunction<VertexDataType, EdgeDataType> callback, RemoteDrunkardCompanion companion) {
    ...
    public void run(int numIterations) throws IOException, RemoteException {
        ...
        if (System.getProperty("membudget") != null)
            memoryBudget = Integer.parseInt(System.getProperty("membudget"));
        engine.setMemoryBudgetMb(memoryBudget);
        engine.setEnableDeterministicExecution(false);
        engine.setAutoLoadNext(false);
        engine.setVertexDataConverter(null);
        // Handle maximum 10M vertices a time.
        engine.setMaxWindow(10000000);
