...
public class GraphChiEngine<VertexDataType, EdgeDataType> {
    ...
    protected java.util.List<SlidingShard<EdgeDataType>> slidingShards;
    ...
    public GraphChiEngine(String baseFilename, int nShards) throws FileNotFoundException, IOException {
        ...
        if (Runtime.getRuntime().maxMemory() < 256 * 1024 * 1024)
            throw new IllegalArgumentException("Java Virtual Machine has only " + memBudget + "bytes maximum memory." + " Please run the JVM with at least 256 megabytes of memory using -Xmx256m. For better performance, use higher value");
    }
    ...
    protected void initializeSlidingShards() throws IOException {
        slidingShards = new org.eclipse.collections.impl.list.mutable.FastList<SlidingShard<EdgeDataType>>();
        for (int p = 0; p < nShards; p++) {
            ...
            SlidingShard<EdgeDataType> slidingShard = new SlidingShard<EdgeDataType>(edataFilename, adjFilename, intervals.get(p).getFirstVertex(), intervals.get(p).getLastVertex());
    ...
    protected MemoryShard<EdgeDataType> createMemoryShard(int intervalStart, int intervalEnd, int execInterval) {
        ...
        MemoryShard<EdgeDataType> newMemoryShard = new MemoryShard<EdgeDataType>(edataFilename, adjFilename, intervals.get(execInterval).getFirstVertex(), intervals.get(execInterval).getLastVertex());
        ...
        for (int iter = 0; iter < niters; iter++) {
            ...
            for (int execInterval = 0; execInterval < nShards; ++execInterval) {
                ...
                if (program instanceof PigGraphChiBase) {
                    ((PigGraphChiBase) program).setStatusString("GraphChi iteration " + iter + " / " + (niters - 1) + ";" + "  vertex interval:" + intervalSt + " -- " + intervalEn);
                }
                    ...
                    if (anyVertexScheduled(subIntervalStart, Math.min(intervalEn, subIntervalStart + adjMaxWindow))) {
                        ...
                        if (!autoLoadNext || nextWindow == null) {
                            ...
                            try {
                                ...
                                if (subIntervalStart != next.getSubInterval().getFirstVertex())
                                    throw new IllegalStateException("Future loaders interval does not match the expected one! " + subIntervalStart + " != " + next.getSubInterval().getFirstVertex());
                        ...
                        if (autoLoadNext) {
                            ...
                            if (subIntervalEnd + 1 <= intervalEn) {
                                nextWindow = new FutureTask<IntervalData>(new AutoLoaderTask(new VertexInterval(subIntervalEnd + 1, Math.min(intervalEn, subIntervalEnd + 1 + adjMaxWindow)), execInterval, memoryShard));
                                ...
                                slidingShards.get(execInterval).setOffset(memoryShard.getStreamingOffset(), memoryShard.getStreamingOffsetVid(), memoryShard.getStreamingOffsetEdgePtr());
                                nextWindow = new FutureTask<IntervalData>(new AutoLoaderTask(new VertexInterval(nextIntervalSt, Math.min(nextIntervalEn, nextIntervalSt + 1 + adjMaxWindow)), execInterval + 1, createMemoryShard(nextIntervalSt, nextIntervalEn, execInterval + 1)));
                            }
                            if (nextWindow != null)
    ...
    private void execUpdates(final GraphChiProgram<VertexDataType, EdgeDataType> program, final ChiVertex<VertexDataType, EdgeDataType>[] vertices) {
    ...
    private void loadBeforeUpdates(int interval, final ChiVertex<VertexDataType, EdgeDataType>[] vertices, final MemoryShard<EdgeDataType> memShard, final int startVertex, final int endVertex) throws IOException {
    ...
    class IntervalData {
        ...
        IntervalData(VertexInterval subInterval, ChiVertex<VertexDataType, EdgeDataType>[] vertices, int vertexBlockId, MemoryShard<EdgeDataType> memShard, int intervalNum) {
        ...
        try {
            ...
            for (int i = 0; i < maxInterval; i++) {
                ...
                if (memReq > memBudget) {
                    ...
                    // Previous vertex was enough
                    return subIntervalStart + i - 1;
                }
            }
            if (totalDegree == 0 && vertexDataConverter == null) {
                throw new NoEdgesInIntervalException();
