...
public class MovieRecommender {
    ...
    protected void execute() throws Exception {
        ...
        DrunkardJob positiveJob = drunkardMobEngine.addJob("positive", EdgeDirection.IN_AND_OUT_EDGES, new PositiveWalkUpdate(), new IntDrunkardCompanion(2, Runtime.getRuntime().maxMemory() / 8));
        DrunkardJob negativeJob = drunkardMobEngine.addJob("negative", EdgeDirection.IN_AND_OUT_EDGES, new NegativeWalkUpdate(), new IntDrunkardCompanion(2, Runtime.getRuntime().maxMemory() / 8));
        ...
        for (int i = 0; i < 500; i++) {
            ...
            System.out.println(i + " avg pos: " + sumEstimatePos / n + "; avg neg: " + sumEstimateNeg / n + "; all=" + allSum / graphInfo.getNumRight() + " (" + (System.currentTimeMillis() - t) + " ms for " + numMovies + " movies");
        }
    }
    ...
    protected static class PositiveWalkUpdate implements WalkUpdateFunction<Integer, Float> {
        ...
        protected static void hopToHighRatings(int[] walks, ChiVertex<Integer, Float> vertex, IntDrunkardContext drunkardContext, Random randomGenerator) {
            int[] hops = WeightedHopper.generateRandomHopsAliasMethod(randomGenerator, vertex, walks.length, EdgeDirection.IN_AND_OUT_EDGES, new WeightedHopper.EdgeWeightMap() {

                // Use exponential weights
                @Override
                public float map(float x) {
                    int r = (int) x;
                    // 2^(rating - 1)     // TODO: should just eliminate negative?
                    return weightedRating[r];
                }
            });
    ...
    protected class NegativeWalkUpdate extends PositiveWalkUpdate {
        ...
        public void processWalksAtVertex(WalkArray walkArray, ChiVertex<Integer, Float> vertex, DrunkardContext drunkardContext_, Random randomGenerator) {
            ...
            if (vertex.numInEdges() > 0 || drunkardContext.getIteration() > 0) {
                ...
                // list of them
                java.util.List<Integer> forwardToPositive = new org.eclipse.collections.impl.list.mutable.FastList<Integer>();
                for (int w : walks) {
